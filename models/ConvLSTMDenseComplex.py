...
class ConvLSTMDenseComplex(GeneralModel):
    ...
    def build(self):
        ...
        x = tfkl.Conv1D(
            64, 3, padding="same", activation=tfkl.LeakyReLU(), name="conv_1"
        )(x)
        x = tfkl.Dropout(0.2)(x)

        x = tfkl.Conv1D(
            64, 3, padding="same", activation=tfkl.LeakyReLU(), name="conv_2"
        )(x)
        x = tfkl.Dropout(0.2)(x)

        x = tfkl.Conv1D(
            128, 3, padding="same", activation=tfkl.LeakyReLU(), name="conv_3"
        )(x)
        x = tfkl.Dropout(0.2)(x)

        x = tfkl.Conv1D(
            128, 3, padding="same", activation=tfkl.LeakyReLU(), name="conv_4"
        )(x)
        x = tfkl.Dropout(0.2)(x)

        x = tfkl.Bidirectional(
            tfkl.LSTM(
                32, return_sequences=True, activation=tfkl.LeakyReLU(), name="lstm_1"
            ),
        ...
        x = tfkl.Bidirectional(
            tfkl.LSTM(32, activation=tfkl.LeakyReLU(), name="lstm_2"),
