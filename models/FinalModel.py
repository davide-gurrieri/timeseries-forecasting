...
fit_param_1 = {
    ...
    "callbacks": [
        ...
        tfk.callbacks.ReduceLROnPlateau(
            monitor="val_loss", patience=7, factor=0.999, mode="min", min_lr=1e-5
        ),
    ],
}
...
class FinalModel(GeneralModel):
    ...
    def build(self):
        ...
        input_layer = tfkl.Input(
            shape=self.build_kwargs["input_shape"], name="input_layer"
        )
        ...
        x = tfkl.Conv1D(
            filters=1, kernel_size=3, padding="same", activation="relu", name="conv1D_2"
        )(x)

        x = tfkl.Flatten(name="flatten")(x)

        output_layer = tfkl.Dense(
            self.build_kwargs["output_shape"], activation="sigmoid", name="output_layer"
        )(x)
