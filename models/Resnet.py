...
class Resnet(GeneralModel):
    ...
    def build(self):
        ...
        dense_layer_a = tfk.layers.Dense(
            2048,
            activation="relu",
            kernel_regularizer=tf.keras.regularizers.L2(1e-5),
            kernel_initializer=tfk.initializers.HeUniform(SEED),
        )(noise_layer)
        dense_layer_b = tfk.layers.Dense(
            1024,
            activation="relu",
            kernel_regularizer=tf.keras.regularizers.L2(1e-5),
            kernel_initializer=tfk.initializers.HeUniform(SEED),
        )(dense_layer_a)
        dense_layer_c = tfk.layers.Dense(
            512,
            activation="relu",
            kernel_regularizer=tf.keras.regularizers.L2(1e-5),
            kernel_initializer=tfk.initializers.HeUniform(SEED),
        )(dense_layer_b)
        bid_1 = tfk.layers.Bidirectional(tfk.layers.LSTM(256, return_sequences=True))(
            dense_layer_c
        )
        bid_2 = tfk.layers.Bidirectional(tfk.layers.LSTM(256, return_sequences=True))(
            bid_1
        )
        ...
        conv_x = tfkl.Conv1D(
            filters=self.build_kwargs["n_feature_maps"], kernel_size=8, padding="same"
        )(dropout)
        conv_x = tfkl.LayerNormalization()(conv_x)
        conv_x = tfkl.Activation("relu")(conv_x)

        conv_y = tfkl.Conv1D(
            filters=self.build_kwargs["n_feature_maps"], kernel_size=5, padding="same"
        )(conv_x)
        conv_y = tfkl.LayerNormalization()(conv_y)
        conv_y = tfkl.Activation("relu")(conv_y)

        conv_z = tfkl.Conv1D(
            filters=self.build_kwargs["n_feature_maps"], kernel_size=3, padding="same"
        )(conv_y)
        conv_z = tfkl.LayerNormalization()(conv_z)

        # expand channels for the sum
        shortcut_y = tfkl.Conv1D(
            filters=self.build_kwargs["n_feature_maps"], kernel_size=1, padding="same"
        )(input_layer)
        ...
        conv_x = tfkl.Conv1D(
            filters=self.build_kwargs["n_feature_maps"] * 2,
            kernel_size=8,
            padding="same",
        )(output_block_1)
        conv_x = tfkl.LayerNormalization()(conv_x)
        ...
        output_layer = tfkl.Dense(self.build_kwargs["output_shape"], activation="relu")(
            dense_layer4
        )
